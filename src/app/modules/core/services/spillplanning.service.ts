...
export class SpillPlanningService {
  ...
  public peakVelocity(q, MAF, D, m, s) {//D units (m), q units (m3/s), MAF units (m3/s)
    ...
    if (m === 'most') {
      if (s && s > 0) {
        let V_p = 0.094 + 0.0143 * Math.pow(this.relativeDrainage(D, MAF), 0.919) * Math.pow(relativeDis, -0.469) * Math.pow(s,0.159) * q / D;
        return V_p;
      } else {
      ...
      if (s && s > 0) {
        let V_mp = 0.25 + 0.02 * Math.pow(this.relativeDrainage(D, MAF), 0.919) * Math.pow(relativeDis, -0.469) * Math.pow(s,0.159) * (q / D);
        return V_mp;
      } else {
  ...
  public peakTimeofTravel(L, q, MAF, D, m, s) { //L units (km), q units (m3/s), MAF units (m3/s), D units (sq m)
    let T: number;
      T = (L * 1000) / this.peakVelocity(q, MAF, D, m, s) * 0.000277778;   
    return T;
  }

  public unitPeakConcentration(L, q, MAF, D, m, s) {
    let C_up: number;
    let relativeDis: number;
    if (q > 0) {
      relativeDis = this.relativeDischarge(q, MAF);
  ...
  public leadingEdge(L, q = 0, MAF, D, m, s) {
    return (this.peakTimeofTravel(L, q, MAF, D, m, s) * 0.89);
  }

  public trailingEdge(L, q = 0, MAF, D, m, s) {
  ...
  public passageTime(L, q = 0, MAF, D, m, s) {
  ...
  public passageTimeTest(L = 0, q = 0, MAF = 0, D = 0, m, s = 0) {
    ...
    let P_v = this.peakVelocity(q, MAF, D, m, s).toFixed(2);
    if (P_v != "1.01") {
    ...
    let P_tot = this.peakTimeofTravel(L, q, MAF, D, m, s).toFixed(1);
    if (P_tot != "28.8") {
    ...
    let C_u = this.unitPeakConcentration(L, q, MAF, D, m, s).toFixed(1);
    if (C_u != "71.9") {
    ...
    let T_l = this.leadingEdge(L, q, MAF, D, m, s).toFixed(1);
    if (T_l != "25.6") {
    ...
    let T_p = this.trailingEdge(L, q, MAF, D, m, s).toFixed(1);
    if (T_p != "7.7") {
    ...
    let TOT = this.passageTime(L, q, MAF, D, m, s).toFixed(1);
    if (TOT != "33.4") {
